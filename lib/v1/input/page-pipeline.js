...
async function runPipeline({ buffer, sniffedType, profile, signal, tempDir, routePage, spawnFn } = {}) {
  ...
  const routeAndRecord = async (pageNum, b64) => {
    ...
    // WR-02 — a result with NO engine means no engine ever produced anything
    // for this page (an exhausted budget, every tier failing). That is a page
    // FAILURE, not a page with empty text: recording it as a success left
    // `error` unset, kept hadError false, and let the job report a clean
    // 'completed' rollup while silently dropping the page's content.
    if (!r || !r.engineId) {
      const err = new Error('no engine produced a result for this page');
      err.code = (r && r.trace && r.trace.stopped_reason) || (r && r.stoppedReason) || 'no_engine_result';
      throw err; // caught by the per-page handler → recorded via pushError
    }
  ...
  const trace = {
    ...
    elapsed_ms: Math.round(clock.monotonicMs() - start),
