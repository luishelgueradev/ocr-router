...
const { pdfPageCount, assertPageCountWithinCap, renderPage } = require('./rasterize');
const { normalizeToFrames } = require('./image-normalize');
...
async function runPipeline({ buffer, sniffedType, profile, signal, tempDir, routePage, spawnFn } = {}) {
  ...
  if (sniffedType === 'application/pdf') {
    ...
    const pageCount = await pdfPageCount(pdfPath, { signal, spawnFn });
    ...
    for (let p = 1; p <= pageCount; p++) {
      try {
        ...
        let png = await renderPage(pdfPath, p, { signal, spawnFn });
    ...
    // Image path — normalize to ordered PNG frames (TIFF/GIF → N frames; HEIC/
    // BMP/PNG/JPEG/WebP → 1). A normalize failure (e.g. unsupported_image_type)
    // propagates so the worker fails the job typed.
    const frames = await normalizeToFrames(buffer, sniffedType, {});
    for (let i = 0; i < frames.length; i++) {
      const p = i + 1;
      try {
        const b64 = frames[i].toString('base64');
        frames[i] = null; // release the frame before routing (INP-07)
        await routeAndRecord(p, b64);
      } catch (err) {
        pushError(p, err);
      }
