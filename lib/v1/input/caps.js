...
const CAPS = Object.freeze({
  ...
  // The image-path equivalent of MAX_PDF_PAGES: max FRAMES we will accept from
  // a multipage TIFF or animated GIF. The frame count comes straight out of an
  // attacker-supplied header, and each frame costs a normalized PNG in memory
  // plus a paid cascade call, so it needs the same pre-decode gate a PDF page
  // count gets. A 10 MB GIF can trivially declare thousands of frames. (frames)
  MAX_IMAGE_FRAMES: intFromEnv('MAX_IMAGE_FRAMES', 50),

  // pdftoppm render resolution. Higher DPI = sharper OCR input but more
  // pixels/memory per page; balanced against MAX_OUTPUT_PIXELS. Applied as the
  // page's natural size at this DPI, clamped to RASTER_MAX_DIM (see
  // rasterize.js#computeScaleTo — poppler's -scale-to overrides -r, so the two
  // are never passed together). (dots/inch)
  ...
  // Long-side pixel CEILING for a rasterized page: the DPI-derived render size
  // is clamped to this, so a hostile MediaBox cannot blow the pixel budget
  // (worst case RASTER_MAX_DIM² pixels). NOT a target — a page whose natural
  // size at RASTER_DPI is smaller is NEVER upscaled to it. Also the long-side
  // resize cap for normalized image frames. (pixels, longest edge)
