...
// An HEIF `ispe` (ImageSpatialExtents) is an ISO-BMFF FullBox:
...
//
// ISO-BMFF (14496-12 §4.2) gives the size field THREE legal encodings, and
// libheif's parser accepts all of them:
//   size >= 8  — the payload follows the type directly (a canonical ispe is 20)
//   size == 1  — a 64-bit `largesize` follows the type; the payload follows that
//   size == 0  — the box runs to end-of-file; the payload follows the type
// G-2: this scan used to accept ONLY `size === 20`, so a bomb that declared its
// extents through a 64-bit extended box — legal, and parsed fine by libheif —
// bypassed the guard completely.
const ISPE_BOX_SIZE = 20;         // a canonical 32-bit ispe box
const ISPE_LARGESIZE_BYTES = 8;   // bytes a 64-bit `largesize` field inserts
 ...
 * it walks every `ispe` occurrence, resolves the payload offset for each legal
 * box-size encoding, and takes the LARGEST declared image (a HEIC carries one
 * per item — typically the primary image plus a thumbnail — and any of them may
 * be decoded).
 *
 * KNOWN LIMIT, stated plainly and NOT narrower than the truth: this is a byte
 * scan, not a parser. It does not walk the box tree, so it cannot tell a real
 * `ispe` from those four bytes appearing inside `mdat` payload, and it cannot
 * follow a dimension declared anywhere it does not recognize. It fails OPEN in
 * both directions — an unrecognized file is allowed through rather than
 * rejected, because failing closed would reject valid HEICs this scan simply
 * did not understand. libheif needs an `ispe` to size its own decode, so a bomb
 * must declare one somewhere, which is what makes the scan worth having; it is
 * best-effort defence in depth, NOT a proof.
 *
 * The residual is bounded by the container memory limit (`mem_limit` in
 * docker-compose.yml — do not remove it believing this guard is sufficient; it
 * is the backstop this comment relies on). Moving the decode into a worker
 * thread with a hard `--max-old-space-size` remains the complete fix and is
 * out of scope here.
 *
...
function assertHeicWithinCap(buf, maxPixels) {
  ...
  while ((i = buf.indexOf('ispe', i, 'ascii')) !== -1) {
    ...
    if (i >= 4) {
      const declaredSize = buf.readUInt32BE(i - 4);

      // Where the FullBox payload begins, for each legal size encoding (G-2).
      // -1 means "this is not a box header we recognize" — skip it.
      let payload = -1;
      if (declaredSize === 1) payload = i + 4 + ISPE_LARGESIZE_BYTES;
      else if (declaredSize === 0 || declaredSize >= ISPE_BOX_SIZE) payload = i + 4;

      // Payload layout: [4] version+flags | [4] width | [4] height. The height
      // read consumes payload+8..payload+11, so the buffer must hold
      // payload+12 — checking only as far as the width (G-1) let a short file
      // read off the end and throw a raw RangeError with no status, which the
      // worker could only map to a 500.
      if (payload >= 0 && payload + 12 <= buf.length) {
        const width = buf.readUInt32BE(payload + 4);
        const height = buf.readUInt32BE(payload + 8);
...
async function* bmpToPngs(buf, maxPixels, maxDim) {
  ...
  let height;
  try {
    // CR-03: runs BEFORE bmp-js Buffer.alloc's from the header. Inside the try
    // for the same reason as the HEIC guard (G-1): it parses attacker-controlled
    // bytes, so it must not be trusted never to throw. Its reads are in bounds
    // for every buffer that clears its own length floor today — this keeps that
    // true by construction rather than by review.
    assertBmpWithinCap(buf, maxPixels);
