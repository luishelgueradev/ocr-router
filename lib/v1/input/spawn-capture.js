...
//   3. Kernel-enforced resource caps via `sh -c 'ulimit -v <KB>; ulimit -t
//      <sec>; exec timeout -s KILL <n>s <cmd> <args>'`. `-v` (address space)
//      is used, NOT `-m` (RSS is unenforced on modern Linux). The `exec` is
//      MANDATORY: without it the shell — not poppler — receives the signal and
//      poppler orphans (Pitfall 4). `timeout(1)` is the hard wall-clock backstop.
...
function spawnCapture(cmd, args = [], {
  ...
  // Build the sandbox body. `exec` replaces the shell so signals reach the
  // target binary directly (Pitfall 4). timeout(1) is the wall-clock backstop.
  ...
  return new Promise((resolve, reject) => {
    const child = spawnFn('/bin/sh', ['-c', body], {
      signal,
      killSignal: 'SIGTERM',
