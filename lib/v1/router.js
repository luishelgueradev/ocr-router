...
const { parseAndCompileSchema } = require('./structured/schema');
const { supportsStructured } = require('./structured/capability');
const { structuredImageSupport } = require('./structured/input-support');

const router = express.Router();

// MODE-05 — allowlist de modos aceptables en la ruta de cascada, DERIVADA del
// catálogo (CATALOG-01: sumar un modo sigue siendo editar lib/models.js y nada
// más). Objeto sin prototipo + Object.hasOwn, mismo patrón que el guard de
// `profile` más abajo: '__proto__' y 'constructor' no son claves propias, así
// que no cuelan (T-02-13).
const CASCADE_MODES = Object.create(null);
for (const m of models) {
  for (const mode of m.modes_supported || []) CASCADE_MODES[mode] = true;
}
// `structured` no vive en modes_supported (es una capability aparte) pero sí es
// un modo válido de la API — se resuelve antes de llegar a la rama de cascada.
const KNOWN_MODES = [...Object.keys(CASCADE_MODES), 'structured'].sort();
...
router.post('/ocr', (req, res) => {
  upload.single('file')(req, res, (err) => {
    ...
    let preset = null;
    let apiKey = null;
    let validate = null; // STR-02 — the compiled schema validator (structured mode)
    const forced = Boolean(modelId);
    const isStructured = typeof requestedMode === 'string' && requestedMode.toLowerCase() === 'structured';

    // mode=describe sólo tiene sentido en un motor de visión, y la cascada puede
    // caer en el motor OCR clásico (que no lo admite). Exigimos `model` explícito
    // para que la capability se resuelva contra el catálogo (CATALOG-01).
    const isDescribe = typeof requestedMode === 'string' && requestedMode.toLowerCase() === 'describe';
    if (isDescribe && !forced) {
      return res.status(422).json({
        error: 'invalid_parameter',
        field: 'model',
        message: 'mode=describe requiere un `model` explícito de visión. Use GET /v1/models para ver cuáles lo admiten.',
      });
    }

    if (isStructured) {
      // STR-02 — compile the client schema (typed 422 field=schema on any issue).
      try {
        ({ validate } = parseAndCompileSchema(req.body?.schema));
      } catch (e) {
        return res.status(e.status || 422).json({
          error: e.code || 'invalid_parameter',
          field: e.field || 'schema',
          message: e.message,
        });
      }

      // D-S9 — structured mode is single-image only this milestone. PDF and
      // multi-frame tiff/gif are rejected typed BEFORE enqueue.
      if (structuredImageSupport(sniffedType) === null) {
        return res.status(422).json({
          error: 'invalid_parameter',
          field: 'file',
          message: 'mode=structured admite una sola imagen (PNG, JPEG, WebP, HEIC o BMP) en esta versión.',
        });
      }
      ...
      if (forced) {
        ...
        if (!model) {
          return res.status(422).json({
            error: 'invalid_parameter', field: 'model',
            message: 'Use GET /v1/models para ver modelos válidos',
          });
        }
        // STR-01 — ocr.space (and any non-structured engine) forced with
        // mode=structured is rejected by CAPABILITY, declaratively.
        if (!supportsStructured(model.id)) {
          return res.status(422).json({
            error: 'invalid_parameter', field: 'model',
            message: `El motor ${model.id} no admite mode=structured. Use GET /v1/models para ver los que sí.`,
          });
        }
      ...
      // Cascade structured path: model stays null; the worker walks the
      // structured-capable chain and fails typed if none is configured.
    } else if (forced) {
      model = findModel(modelId);
      if (!model) {
        return res.status(422).json({
      ...
      // are handled inside the runner). MODE-05: informativo NO es "cualquier
      // cosa". Antes un modo desconocido se normalizaba y seguía de largo, así
      // que un `structrued` mal tipeado encolaba, gastaba proveedor y devolvía
      // texto plano donde el cliente esperaba JSON — no rompía, mentía.
      // Validamos contra la allowlist ANTES de encolar.
      if (requestedMode === undefined || requestedMode === null || requestedMode === '') {
        mode = 'quality';
      } else {
        const candidate = typeof requestedMode === 'string' ? requestedMode.toLowerCase() : null;
        if (candidate === null || !Object.hasOwn(CASCADE_MODES, candidate)) {
          return res.status(422).json({
            error: 'invalid_parameter',
            field: 'mode',
            message: `Modo desconocido. Modos válidos: ${KNOWN_MODES.join(', ')}.`,
          });
        }
        mode = candidate;
      }
...
router.get('/models', (req, res) => {
  return res.status(200).json({
    models: models.map(m => ({
      ...
      // STR-01 — advertise structured capability so a client can discover which
      // engines accept mode=structured (declarative, from the capability table).
      supports_structured: supportsStructured(m.id),
    })),
