...
function drainAndCancel(timeoutMs = 35000, deps = {}) {
  ...
  pending = (async () => {
    try {
      ...
      // leaks nothing. Wrapped in its own try/catch: a temp-rm failure must
      // NEVER prevent the job drain below (cleanup is best-effort, the job
      // drain is the contract). Injectable for tests via deps.temp.
      try {
