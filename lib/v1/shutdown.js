  ...
  pending = (async () => {
    try {
      ...
      try {
        ...
        // WR-06 — the losing timer MUST be cleared. Left armed, a clean fast
        // drain still held the event loop for the whole remaining grace window
        // (35 s by default), so every `docker stop` took ~35 s no matter how
        // quickly work finished — and any orchestrator with a shorter stop grace
        // SIGKILLed us, which is the exact outcome this drain exists to avoid.
        //
        // Deliberately NOT unref'd: this timer is the only thing keeping the
        // loop alive while we wait on limiter.stop(), so unref'ing it would let
        // the process exit before the timeout branch could fail the in-flight
        // jobs with shutdown_timeout. clearTimeout on the winning path is what
        // releases the loop, and it is sufficient.
        let timer;
        ...
        logger.info(
          { duration_ms: Math.round(clock.monotonicMs() - start), cancelled: cancelled.length, drained: winner === 'drained' },
          'shutdown_complete'
        );
      } catch (e) {
