...
function makeCascadeRoutePage({ profileName, controller, deadline }) {
  return async (pageBase64, mimeType) => {
    ...
    // An unclamped `deadline - Date.now()` goes NEGATIVE after MAX_JOB_MS, which
...
async function runInputJob(jobId, { buffer, sniffedType, mode, profile, model, forced, apiKey, preset }, jobLogger, spawnFn) {
  ...
  const controller = new AbortController();
  const deadline = Date.now() + CAPS.MAX_JOB_MS;
...
async function runJob(jobId, { model, buffer, mimeType, mode, apiKey, preset, requestId, profile, forced, spawnFn }) {
...
module.exports = { runJob, runInputJob, limiter, MAX_QUEUE_DEPTH };
