...
// ollama-minimax-m3, ollama-gemma4-31b, ollama-qwen35-397b).
...
const CONFIG = {
  ...
  profiles: {
    fast: {
      chain: ['ocrspace-engine2', 'ollama-minimax-m3'],
      threshold: floatFromEnv('PROFILE_FAST_THRESHOLD', 0.50),
      maxAttempts: 2,
      budgetMs: 30000,
    },
    balanced: {
      chain: ['ocrspace-engine2', 'ollama-minimax-m3', 'ollama-gemma4-31b'],
      threshold: floatFromEnv('PROFILE_BALANCED_THRESHOLD', 0.60),
      maxAttempts: 3,
      budgetMs: 60000,
    },
    quality: {
      chain: ['ocrspace-engine2', 'ollama-minimax-m3', 'ollama-gemma4-31b', 'ollama-qwen35-397b'],
  ...
  // --- Engine capability table (D-07 / CASC-06 / STR-01) --------------------
  // Keyed by engine id. `supports_structured` gates mode=structured (Phase 4):
  // the vision LLMs (Ollama) do schema-constrained decoding; ocr.space is a
  // classic OCR engine with no JSON-schema capability, so it stays false and is
  // excluded from every structured chain (and rejected when force-selected with
  // mode=structured). This is DECLARATIVE — no engine-id branching downstream.
  capabilities: {
    'ocrspace-engine2':      { provider: 'ocrspace', modes: ['quality'],           supports_structured: false },
    'ollama-minimax-m3': { provider: 'ollama',   modes: ['speed', 'quality'],  supports_structured: true  },
    'ollama-gemma4-31b':     { provider: 'ollama',   modes: ['speed', 'quality'],  supports_structured: true  },
    'ollama-qwen35-397b':  { provider: 'ollama',   modes: ['speed', 'quality'],  supports_structured: true  },
  },
