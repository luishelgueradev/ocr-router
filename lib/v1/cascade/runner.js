...
async function runCascade({ base64, mimeType, profile, deadlineSignal, budgetMs } = {}) {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    const perEngineMs = Math.min(CONFIG.perEngineMs, remaining);
    ...
    const timeMs = Date.now() - engStart;
    ...
    if (!res || res.ok === false) {
      ...
      // Ollama same-account quota (429/quota): skip remaining same-provider
      // tiers (record them) and stop — graceful degradation, not a dead-quota
      // hammer (D-12 / Pitfall 2).
      if (QUOTA_CODES.has(code)) {
        skippedProviders.add(model.provider);
        for (let j = i + 1; j < chain.length; j++) {
          if (chain[j].provider === model.provider) {
            recordAttempt(trace, {
              engine: chain[j].id, provider: chain[j].provider, outcome: 'skipped',
              confidence: null, timeMs: 0, error: null, reason: 'provider_quota',
            });
          }
        }
    ...
    if (passesThreshold(confidence, profileName)) {
      ...
      finalizeTrace(trace, {
        ...
        stoppedReason: 'passed', elapsedMs: Date.now() - start,
      });
