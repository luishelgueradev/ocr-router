...
- **D-02 (refined after research):** Fall-through-on-low-confidence uses a **multi-signal heuristic** `computeConfidence(text, {overlay}) → [0,1]` with two **hard gates** (empty text → 0; garbage/non-printable ratio > 0.30 → 0) plus a **weighted soft score** (printable-ratio 0.5 / length 0.3 / overlay-word-count 0.2, renormalized when overlay absent). Starting thresholds `fast 0.50 / balanced 0.60 / quality 0.70`, all **env-overridable** (tuning needs no code change). Signals: text length, garbage/non-printable ratio, and the weak overlay word-count proxy (per D-03 — NOT a real confidence). Keep the logic **pure + unit-tested** against the research's 10-fixture clean/garbage/empty matrix (STATE risk-flag). Threshold *numbers* are LOW-confidence until a **live-key calibration spike** (deferred to PENDING-ISSUES / human ops checkpoint) — the defaults are defensible and sanity-checked (clean scanned doc ≈0.995 → stops at ocr.space tier-1 per SC#1; garbage → 0 → escalates).
- **D-03 (AMENDED after research):** ocr.space returns **NO confidence score at any level** — verified 3× against official docs. `isOverlayRequired=true` yields only bounding-box **geometry** (`WordText/Left/Top/Height/Width` + `HasOverlay`), no per-word or mean confidence. Therefore the heuristic is **uniform across all engines** (classic-OCR and LLM), judged on text-length + garbage-ratio. The only overlay-derived signal is **detected word count**, used as a **low-weight positive proxy** (not a confidence). Enabling overlay is optional and only worth it for that weak word-count signal; keep any overlay data **internal** to the heuristic (never in the public API — REQUIREMENTS out-of-scope on unified overlay). *(This supersedes the original D-03 and the PROJECT "known detail" assumption that overlay yields confidence — research A1.)*
